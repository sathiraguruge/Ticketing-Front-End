...
export default class NetworkReports extends Component {
    constructor(props) {
        super(props);
        this.state = {
            busNo: '',
    ...
    componentDidMount() {
        ...
        // Load the distribution over a week
        this.chartServices.getPassengerDistributionOverAWeek().then(response => {
            this.setState({daysOfTheWeekData: response.data});
        }).catch(function (error) {
            console.log(error);
        })
    }
    ...
    render() {
        ...
        let daysOfTheWeek = {
            ...
            datasets: [
                {
                    label: 'Load',
                    backgroundColor: 'rgba(254,93,39,1)',
                    borderColor: 'rgba(0,0,0,0)',
                    borderWidth: 1,
                    data: this.state.daysOfTheWeekData.data
                }
            ]
        }
                ...
                <div className="container p-2" style={{paddingBottom: '500px'}}>
                    <form onSubmit={this.onSubmit}>
                        <QueueAnim duration="1000" interval="400">
                            <span className="contact100-form-title float-lg-left">Network Statistics</span>
                            <br/><br/><br/><br/>
                            ...
                            {/*This is the chart for passenger distribution over the day*/}
                            <div>
                                <Bar
                                    data={timeOfDayChart}
                                    options={{
                                        title:{
                                            display:true,
                                            text: this.state.timeOfDayChartData.name,
                                            fontSize:20
                                        },
                                        legend:{
                                            display:true,
                            ...
                            {/*This is the chart for passenger distribution over the week*/}
                            <div>
                                <Bar
                                    data={daysOfTheWeek}
                                    options={{
                                        title:{
                                            display:true,
                                            text: this.state.daysOfTheWeekData.name,
                                            fontSize:20
                                        },
                                        legend:{
                                            display:true,
